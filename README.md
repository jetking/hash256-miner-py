...
不指定 `--global-size` / `--local-size` 时，工具会按设备能力自动选取：
`local = min(256, device.max_work_group_size)`、`global = max(4M, compute_units × local × HASH256_OVER_SUBSCRIBE)`（默认乘子 256）。中端 GPU 落在 4M 兜底；高端多 CU 的卡会自动放大到 8M+。

`--global-size N` 是每次内核启动调度的 work-item 数。要手动设：高端显卡可以调高（8M 到 32M），笔记本可以调低。

`--local-size` 是 OpenCL 工作组大小。256 是安全默认值；某些硬件更适合 64 或 128。

实际每批的 keccak 哈希数 = `global_size × HASH256_NONCES_PER_ITEM`。`HASH256_NONCES_PER_ITEM`（默认 64，2 的幂，取值 1–256）让每个 work-item 在内核内连续扫多个 nonce，复用 challenge 与 nonce 高位寄存器；不同 GPU 的最佳值不同，建议用 `benchmark` sweep：

```bash
for n in 16 32 64 128; do
    echo "=== N=$n ==="
    HASH256_NONCES_PER_ITEM=$n hash256-miner benchmark --seconds 30
done
```

`--batch-cooldown-seconds` 会在两个 GPU 批次之间暂停一小段时间。Windows 上如果出现风扇突然拉满、屏幕短暂黑屏、随后报
`INVALID_COMMAND_QUEUE`，通常是显卡驱动重置了 OpenCL 上下文。先降低单批工作量，再按需加冷却间隔，例如：

```bash
hash256-miner mine ... --global-size 1048576 --batch-cooldown-seconds 0.05
```

如果仍然黑屏，继续把 `--global-size` 降到 `262144` 或 `65536`，并检查显卡温度、功耗限制和驱动版本。
